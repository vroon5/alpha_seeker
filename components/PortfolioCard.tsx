    ...
    >
      ...
      <div className="p-4 border-b border-gray-700 flex justify-between items-start bg-gray-800/30">
        <div className="flex gap-3">
          <img src={portfolio.kolAvatar} alt={portfolio.kolName} className="w-10 h-10 rounded-full border border-gray-600 object-cover" />
          <div>
            ...
            <div className="flex items-center gap-1 text-xs text-gray-400">
              ...
              {/* Fixed: Changed isVerifiedKOL to isVerified to match Portfolio type */}
              {portfolio.isVerified && <Icons.ShieldCheck size={12} className="text-alpha-accent" />}
            </div>
          </div>
        </div>
      ...
      <div className="p-4 grid grid-cols-2 gap-2">
        {portfolio.stocks.map((stock) => (
          <div key={stock.ticker} className="bg-gray-800/50 p-2 rounded-lg flex justify-between items-center border border-gray-700/50 hover:border-gray-600 transition-colors">
      ...
      {/* Footer / Actions */}
      <div className="p-4 border-t border-gray-700 space-y-3 bg-gray-900/20">
        {/* AI Analysis Section */}
        {aiAnalysis ? (
          <div className="p-3 bg-indigo-900/20 border border-indigo-500/20 rounded-lg animate-in fade-in" onClick={(e) => e.stopPropagation()}>
             ...
             <p className="text-[11px] text-gray-300 leading-relaxed line-clamp-2">{aiAnalysis}</p>
          </div>
        ) : (
          <button 
            ...
            <>
                ...
                <button 
                    ...
                    isBacked 
                        ...
                        : 'bg-alpha-success hover:bg-green-600 text-white shadow-lg shadow-green-900/20 active:scale-95'
                    }`}
                >
